...
# Get list of folders
parent_dir <- "Data/Genesys/Characterization_and_evalution_datasets"  # location in Drive 
folders <- list.dirs(parent_dir, recursive = FALSE)
all_info <- list()  # initialize list to hold results

# Loop through folders to process Excel files
for (folder in folders) {
  files <- list.files(folder, pattern = "\\.xlsx?$", full.names = TRUE)
  crop_name <- basename(folder)
  
  if (length(files) == 0) {
    # Add placeholder if no datasets found
    all_info[[crop_name]] <- data.frame(
      cropstrategy = crop_name,
      File = NA,
      Rows = NA,
      stringsAsFactors = FALSE
    )
  } else {
    # Create row info per file
...
# Combine all folder-level data
final_df <- do.call(rbind, all_info)

# Calculate metrics 
char_eval_summary <- final_df %>%
  group_by(cropstrategy) %>%
  summarize(
    char_eval_datasets_count = sum(!is.na(File)),
    accessions_char_eval_datasets_count = if (all(is.na(Rows))) NA else sum(Rows, na.rm = TRUE),
    .groups = "drop"
  )

# Print summary
cat("\n📊 Characterization Evaluation Summary:\n")
print(char_eval_summary)

# Save
write.csv(apply(char_eval_summary, 2, as.character),"char_eval_metric_results.csv", row.names = FALSE)
